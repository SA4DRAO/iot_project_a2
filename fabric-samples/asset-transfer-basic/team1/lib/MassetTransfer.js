...
class AssetTransfer extends Contract {
    ...
    constructor() {
        super('AssetTransfer');
        this.mongoUrl = 'mongodb://localhost:27017'; // Update with your MongoDB connection string
        this.mongoDbName = 'sensor'; // Update with your MongoDB database name
        this.mongoCollectionName = 'assets'; // Update with your MongoDB collection name
    }

    // CreateAsset issues a new asset to the world state with given details.
    async CreateAsset(ctx, rid, sid, temp, light, air, humid, time) {
        ...
        const client = new MongoClient(this.mongoUrl, { useNewUrlParser: true, useUnifiedTopology: true });
        try {
            await client.connect();
            const db = client.db(this.mongoDbName);
            const collection = db.collection(this.mongoCollectionName);
    ...
    async UpdateAsset(ctx, rid, sid, temp, light, air, humid, time) {
        const exists = await this.AssetExists(ctx, rid);
        if (!exists) {
            throw new Error(`The asset ${rid} does not exist`);
        }
