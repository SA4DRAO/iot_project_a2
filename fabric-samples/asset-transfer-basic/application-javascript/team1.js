...
const chaincodeName = process.env.CHAINCODE_NAME || 'basic1';  // write my chaincode name 
...
async function main() {
	try {
		...
		try {
			...
			let row_id = 11750; // Initialize row_id
			let currentIndex = 0;
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  const temperature = jsonData.rsc.temperature.value;
				  const light = jsonData.rsc.light.value;
				  const airQuality = jsonData.rsc['air-quality'].value;
          			  const humidity = jsonData.rsc.humidity.value;
				  ...
				  db.collection('team1').insertOne(asset,function(err, collection){
					if (err) throw err;
					console.log("Record inserted Successfully");
						
				  });
				  console.log('\n--> Submit Transaction: CreateAsset, creates a new asset with RID, SID, Temperature, Light, Air-quality, Humidity, time');
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), temperature.toString(), light.toString(), airQuality.toString(), humidity.toString(), currentTime, team);
				  console.log('*** Result: committed');
				  if (`${result}` !== '') {
