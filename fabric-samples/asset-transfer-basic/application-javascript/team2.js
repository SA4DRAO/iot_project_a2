...
const chaincodeName = process.env.CHAINCODE_NAME || 'basic2';  // write my chaincode name 
...
async function main() {
	try {
		...
		try {
			...
			let row_id = 12650; // Initialize row_id
			let currentIndex = 0;
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  db.collection('team2').insertOne(asset,function(err, collection){
					if (err) throw err;
				  ...
				  console.log('\n--> Submit Transaction: CreateAsset, creates a new asset with RID, SID, Temperature');
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), temperature.toString(), team, currentTime);
				  console.log('*** Result: committed');
				  if (`${result}` !== '') {
