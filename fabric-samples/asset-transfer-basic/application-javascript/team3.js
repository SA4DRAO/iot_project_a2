...
const urls = [
  'http://[aaaa::c30c:0:0:d]/',
  'http://[aaaa::c30c:0:0:e]/'
];
...
const chaincodeName = process.env.CHAINCODE_NAME || 'basic3';  // write my chaincode name 
...
async function main() {
	try {
		...
		try {
			...
			let row_id = 14700; // Initialize row_id
			let currentIndex = 0;
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  const bpm = jsonData.rsc.bpm.value;
				  const hrv = jsonData.rsc.hrv.value;
				  ...
				  db.collection('team3').insertOne(asset,function(err, collection){
					if (err) throw err;
					console.log("Record inserted Successfully");
						
				  });
				  console.log('\n--> Submit Transaction: CreateAsset, creates a new asset with RID, SID, bpm, hrv, time');
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), bpm.toString(), hrv.toString(), currentTime, team);
				  console.log('* Result: committed');
				  if (`${result}` !== '') {
				    console.log(`* Result: ${prettyJSONString(result.toString())}`);
				  }
				  console.log('\n--> GET ALL ASSETS\n');
				  const result2 = await contract.submitTransaction('GetAllAssets'); 
