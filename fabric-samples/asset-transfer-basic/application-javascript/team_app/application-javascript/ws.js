			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  const co = jsonData.rsc.co.value;
				  const co2 = jsonData.rsc.co2.value;
          			  const so2 = jsonData.rsc.so2.value;
          			  const pm = jsonData.rsc.pm.value;
				  ...
				  const result = await contract.submitTransaction('CreateAsset', id.toString(), temp.toString(), co.toString(), co2.toString(), so2.toString(), pm.toString(), humid.toString());
