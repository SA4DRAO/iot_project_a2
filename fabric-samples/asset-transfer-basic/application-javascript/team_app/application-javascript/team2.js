...
mongoose.connect('mongodb://localhost:27017/team2_data');
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), temperature.toString(), team);
				  console.log('*** Result: committed');
				  if (`${result}` !== '') {
