...
const urls = [
  'http://[aaaa::c30c:0:0:f]/',
  'http://[aaaa::c30c:0:0:10]/'
];
...
const chaincodeName = process.env.CHAINCODE_NAME || 'basic5';  // write my chaincode name 

const mspOrg1 = 'Org1MSP';
const walletPath = path.join(__dirname, 'wallet');
const org1UserId = 'javascriptAppUser5';
...
async function main() {
	try {
		...
		try {
			...
			let row_id = 15500; // Initialize row_id
			let currentIndex = 0;
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
                  ...
                  const equipment_id = jsonData.rsc.equipment_id.value;
                  const rfid = jsonData.rsc.rfid.value;
				  ...
				  db.collection('team5').insertOne(asset,function(err, collection){
					if (err) throw err;
					console.log("Record inserted Successfully");
						
				  });
				  console.log('\n--> Submit Transaction: CreateAsset, creates a new asset with RID, SID, Latitude, Longitude, Power, Equipment ID, RFID, time');
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), latitude.toString(), longitude.toString(), power.toString(), equipment_id.toString(), rfid.toString(), currentTime, team);
				  console.log('* Result: committed');
				  if (`${result}` !== '') {
