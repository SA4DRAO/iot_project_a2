...
const org2UserId = 'javascriptAppUser';
...
async function main() {
	try {
		...
		try {
			...
			let row_id = 222; // Initialize row_id
			let currentIndex = 0;
			...
			async function fetchAndSubmitData(url, index) {
			  http.get(url, async (response) => {
				...
				if (response.statusCode === 200) {
				  ...
				  const result = await contract.submitTransaction('CreateAsset', row_id.toString(), index.toString(), temperature.toString(), light.toString(), airQuality.toString(), humidity.toString(), currentTime);
				  console.log('*** Result: committed');
				  if (`${result}` !== '') {
